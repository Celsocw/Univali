<?php
	...
	if($thisMD5 != $prevMD5) {
		// set up tables
		setupTable('Pessoas', []);
		setupIndexes('Pessoas', ['Estado','Cidade','Bairro','profissao',]);

		setupTable('Eleitores', []);
		setupIndexes('Eleitores', ['Nome','Zona','secao','partido','indicacao',]);

		setupTable('Candidatos', []);
		setupIndexes('Candidatos', ['Partido','Nome',]);

		setupTable('Colaboradores', []);
		setupIndexes('Colaboradores', ['Nome','Profissao','Funcao','Responsavel',]);

		setupTable('Partidos', []);

		setupTable('Profissoes', []);

		setupTable('Funcoes', []);

		setupTable('Estados', []);

		setupTable('Municipios', []);
		setupIndexes('Municipios', ['Estado',]);

		setupTable('Bairro', []);
		setupIndexes('Bairro', ['Cidade',]);

		setupTable('Zona', []);
		setupIndexes('Zona', ['Estado','Municipio',]);

		setupTable('Secao', []);
		setupIndexes('Secao', ['Cidade','Bairro','Zona',]);

		setupTable('Agenda', []);
		setupIndexes('Agenda', ['Responsavel',]);

		setupTable('Atendimentos', []);
		setupIndexes('Atendimentos', ['Colaborador','Eleitor',]);
