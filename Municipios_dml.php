...
function Municipios_insert(&$error_message = '') {
	...
	if(function_exists('Municipios_after_insert')) {
		$res = sql("SELECT * FROM `Municipios` WHERE `Id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
	...
	set_record_owner('Municipios', $recID, $recordOwner);
...
function Municipios_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	// child table: Pessoas
	$res = sql("SELECT `Id` FROM `Municipios` WHERE `Id`='{$selected_id}'", $eo);
	$Id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Pessoas` WHERE `Cidade`='" . makeSafe($Id[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'Pessoas', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'Municipios_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'Municipios_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `Id` FROM `Municipios` WHERE `Id`='{$selected_id}'", $eo);
	$Id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Bairro` WHERE `Cidade`='" . makeSafe($Id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'Bairro', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `Id` FROM `Municipios` WHERE `Id`='{$selected_id}'", $eo);
	$Id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Zona` WHERE `Municipio`='" . makeSafe($Id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `Municipios` WHERE `Id`='{$selected_id}'", $eo);
...
function Municipios_update(&$selected_id, &$error_message = '') {
	...
	if(function_exists('Municipios_after_update')) {
		$res = sql("SELECT * FROM `Municipios` WHERE `Id`='{$data['selectedID']}' LIMIT 1", $eo);
	...
	set_record_owner('Municipios', $selected_id);
}

function Municipios_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('Municipios', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('Municipios', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('Municipios', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		$res = sql("SELECT * FROM `Municipios` WHERE `Id`='" . makeSafe($selectedId) . "'", $eo);
			...
			$j("#Estado-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_Estado__RAND__.value, t: 'Municipios', f: 'Estado' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'Municipios', f: 'Estado' }; },
	...
	// process buttons
	if($showSaveNew) {
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return Municipios_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
	...
	$lookup_fields = ['Estado' => ['Estados', 'Estado'], ];
	...
	if($hasSelectedId) {
		$jdata = get_joined_record('Municipios', $selectedId);
	...
	if(function_exists('Municipios_dv')) {
		$args = [];
		Municipios_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
